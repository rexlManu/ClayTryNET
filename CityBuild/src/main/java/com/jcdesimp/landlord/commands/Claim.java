...
public class Claim implements LandlordCommand {
    ...
    public boolean execute(CommandSender sender, String[] args, String label) {
        ...
        if (!(sender instanceof Player)) {
            ...
            if (amt > 0) {
                ...
                } else if (!plugin.getvHandler().hasEnoughMoney(player.getUniqueId(), amt)) {
                    player.sendMessage(ChatColor.RED + claimPrice.replace("#{cost}", amt + ""));
                    return true;
                } else {
                    player.sendMessage(ChatColor.YELLOW + charged.replace("#{cost}", amt + ""));
                    plugin.getvHandler().removeMoney(player.getUniqueId(), amt);
                }
            }
    ...
    public String getHelpText(CommandSender sender) {
        ...
        //if (plugin.hasVault()) {
        if (plugin.getConfig().getDouble("economy.buyPrice", 100.0) > 0) {     //conf
            helpString += ChatColor.YELLOW + " " + ChatColor.ITALIC + priceWarning
                    .replace(
                            "#{pricetag}",                 // insert the formatted price string
                            plugin.getConfig().getDouble("economy.buyPrice", 100.0) + "");
        }
        //}
