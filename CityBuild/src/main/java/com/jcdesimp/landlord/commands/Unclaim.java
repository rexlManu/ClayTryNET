...
public class Unclaim implements LandlordCommand {
        ...
        if (!(sender instanceof Player)) {
            ...
            if (args.length > 1) {
                try {
                    ...
                    if (args.length > 2) {
                        ...
                        if (plugin.getCityBuild().getServer().getWorld(worldname) == null) {
            ...
            if (amt > 0) {
                ...
                if (numFree > 0 && plugin.getDatabase().find(OwnedLand.class).where().eq("ownerName", player.getUniqueId().toString()).findRowCount() <= numFree) {
                    ...
                    plugin.getvHandler().addMoney(player.getUniqueId(), amt);
                    player.sendMessage(ChatColor.GREEN + landSold.replace("#{amount}", (amt) + ""));
                    //return true;
                }
            }
            ...
            dbLand.highlightLand(player, Effect.SPELL);
    ...
    public String getHelpText(CommandSender sender) {
       ...
       //if (plugin.hasVault()) {
            if (plugin.getConfig().getDouble("economy.sellPrice", 50.0) > 0) { //conf
                helpString += ChatColor.YELLOW + " " + ChatColor.ITALIC + priceWarning
                        .replace(
                                "#{pricetag}",                  // insert the formatted price string
                               ""+(plugin.getConfig().getDouble("economy.sellPrice", 50.0))        //conf
                        );
            }
       // }
