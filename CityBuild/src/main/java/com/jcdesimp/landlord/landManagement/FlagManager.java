...
public class FlagManager {
    ...
    public boolean registerFlag(Landflag f) {
        if (registeredFlags.containsKey(f.getClass().getSimpleName())) {
            plugin.getCityBuild().getLogger().warning("Could not register flag \"" + f.getClass().getSimpleName() + "\" because a flag is already registered with that name!");
        ...
        if (lfp == null) {
            plugin.getCityBuild().getLogger().info("Registering new land flag: " + f.getClass().getSimpleName());
        ...
        try {
            plugin.getCityBuild().getServer().getPluginManager().registerEvents(f, plugin.getCityBuild());
            registeredFlags.put(f.getClass().getSimpleName(), f);
        } catch (Exception e) {
            plugin.getCityBuild().getLogger().warning("Error occured while registering flag \"" + f.getClass().getSimpleName() + "\":");
            e.printStackTrace();
            return false;
        }

        plugin.getCityBuild().getLogger().info("Registered flag: " + f.getClass().getSimpleName());
        return true;
