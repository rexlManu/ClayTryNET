 ...
 */
package net.claytry.lobbymodule.scoreboard;
...
import net.claytry.lobbymodule.playtime.PlayTime;
...
public class PlayerScoreboard extends BukkitRunnable {
    ...
    public PlayerScoreboard() {
        runTaskTimer(BukkitPlayerBridge.getInstance().getJavaPlugin(), 0, 100);
        this.scoreboardTitle = "§8» §2§lC§alayTry §8┃ §7Lobby";
    ...
    public void run() {
        ...
        }.runTaskTimer(BukkitPlayerBridge.getInstance().getJavaPlugin(), 0, 2);
    }
    ...
    public void updateRank(Player player) {
        final CloudPlayer onlinePlayer = CloudAPI.getInstance().getOnlinePlayer(player.getUniqueId());
        player.getScoreboard().getTeam("rank").setSuffix("§7" + onlinePlayer.getPermissionEntity().getHighestPermissionGroup(CloudAPI.getInstance().getPermissionPool()).getSuffix());
        player.setScoreboard(player.getScoreboard());
    }
    ...
    public void sendScoreboard(Player player) {
        ...
        objective.getScore(" §2✦ §8┃ §aDein Rang").setScore(9);
//        objective.getScore("  §8» §7Admin").setScore(8);
        createTeam(8, "rank", "  §8» ", "§7Loading", objective, scoreboard, "§e");
